...
export function AuthProvider({ children }: AuthProviderProps) {
  ...
  const syncEmailUser = useCallback(async (user: AuthUser | null) => {
    if (!user && authSwitchRef.current === 'subscription') {
      return;
    }
    if (user) {
      await clearPersistedSubscriptionSession().catch(() => null);
    }
    updateState(buildEmailState(user));
  }, [updateState]);
  ...
  const loginWithPassword = useCallback(async (email: string, password: string) => {
    ...
    try {
      ...
      if (result?.user) {
        await syncEmailUser(result.user);
      }
      return result;
    } catch (error) {
  ...
  }, [setOperationLoading, syncEmailUser]);

  const loginWithSubscriptionCode = useCallback(async (code: string) => {
    setOperationLoading(true);
    try {
      authSwitchRef.current = 'subscription';
      ...
      setTimeout(() => {
        if (authSwitchRef.current === 'subscription') {
          authSwitchRef.current = null;
        }
      }, 4000);
  ...
  const verifyOTPAndLogin = useCallback(async (...args: Parameters<typeof authService.verifyOTPAndLogin>) => {
    setOperationLoading(true);
    try {
      return await authService.verifyOTPAndLogin(...args);
    } finally {
      setOperationLoading(false);
    }
  }, [setOperationLoading]);
  ...
  }, [bootstrapAuthState, syncEmailUser]);
